...
class _SearchScreenState extends State<SearchScreen> with SingleTickerProviderStateMixin {
  ...
  void _onSearchSubmitted(String query) {
    ...
    if (query.trim().length < 3) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('يرجى كتابة 3 أحرف على الأقل للبحث')),
      );
      return;
    }
  ...
  Widget build(BuildContext context) {
    ...
    // Determine if we should show the search view (either typing or focused)
    final bool showSearchView = provider.isSearching || _isSearchFieldFocused;
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          _buildSearchBar(context, isDark, provider, showSearchView),
  ...
  Widget _buildSearchBar(BuildContext context, bool isDark, SearchProvider provider, bool showSearchView) {
    return Container(
      ...
      child: TextField(
        ...
        decoration: InputDecoration(
          ...
          prefixIcon: showSearchView ? IconButton(
  ...
  Widget _buildSearchResults(SearchProvider provider) {
              ...
              children: [
                ...
                TextButton(
                  onPressed: () async {
                    ...
                    await LocalDbService.instance.clearUser(); // Or a specific method if you add one
                    _loadRecentSearches();
                  },
    ...
    return ListView.builder(
      ...
      itemCount: provider.userSearchResults.length + (provider.hasMoreUsers ? 1 : 0),
      itemBuilder: (context, index) {
        if (index < provider.userSearchResults.length) {
